...
export default function App() {
  ...
  const [gameState, setGameState] = useState<'SPLASH' | 'HOME' | 'PLAYING'>('SPLASH');

  useEffect(() => {
    if (gameState === 'SPLASH') {
      const timer = setTimeout(() => setGameState('HOME'), 4000);
  ...
  const initThree = useCallback(() => {
    ...
    // scene.background = new THREE.Color(0x020617); // Removed for transparency
    ...
    // Force style on domElement just in case
    ...
    loader.load(
      ...
      (gltf) => {
        ...
        // Adjust scale and rotation to match the game's coordinate system
        // User requested larger scale and rotation alignment
        model.scale.set(4.0, 4.0, 4.0);
        model.rotation.y = Math.PI / 2; // Quarter turn (90 degrees) to face forward

        // Fix materials if needed (make them responsive to light)
      ...
      (error) => {
        ...
        // Fallback: Add a simple box if load fails so game is playable
        const box = new THREE.Mesh(new THREE.BoxGeometry(2, 1, 3), new THREE.MeshStandardMaterial({ color: 0xff0000 }));
        shipGroup.add(box);
      }
    );
    ...
    // Shield (Attached to shipGroup so it moves with it)
    const shieldGeom = new THREE.IcosahedronGeometry(3.0, 2);
    ...
    // Thruster Lights (Attached to shipGroup)
    const tLightL = new THREE.PointLight(0x06b6d4, 0, 8);
    tLightL.position.set(-0.5, 0.5, 2); // Position relative to new model
    shipGroup.add(tLightL);
    g.thrusterLightL = tLightL;

    const tLightR = new THREE.PointLight(0x06b6d4, 0, 8);
    tLightR.position.set(0.5, 0.5, 2);
  ...
  useEffect(() => {
    if (gameRef.current.ship) {
      gameRef.current.ship.scale.set(playerScale, playerScale, playerScale);
    }
  }, [playerScale]);
  ...
  useEffect(() => {
    ...
    const gameLoop = (time: number) => {
      ...
      if (g.shipState.alive && g.ship && g.camera && g.scene) {
        // Idle rotation in HOME menu
        if (gameState === 'HOME') {
          g.ship.rotation.y += 0.005;
          g.ship.rotation.z = Math.sin(time * 0.001) * 0.05;
          // Ensure camera is looking at ship
          g.camera.lookAt(g.ship.position);
          g.camera.position.lerp(new THREE.Vector3(0, 30, 40), 0.05);
        }
      ...
      if (gameState === 'PLAYING') {
        ...
        if (g.asteroids.length === 0 && g.shipState.alive && gameState === 'PLAYING') spawnWave(g.wave + 1);
  ...
  return (
    <div className="fixed inset-0 flex flex-col p-4 md:p-6 bg-transparent text-slate-200 overflow-hidden touch-none select-none">
      <AnimatePresence>
        {showSidebar && (
          <MotionDiv
            initial={{ opacity: 0 }}
        ...
        {gameState === 'SPLASH' && (
          ...
          >
            <img
              ...
              alt="Gorboy Loading"
              className="max-w-[80%] max-h-[60%] mb-8 pixelated"
              style={{ imageRendering: 'pixelated' }}
            />
            <div className="text-2xl font-mono text-white animate-pulse tracking-widest uppercase">
              Loading...
            </div>
          </MotionDiv>
        )}
      ...
      <AnimatePresence>
        {gameState === 'HOME' && (
          ...
          >
            <div className="pointer-events-auto flex flex-col items-center gap-8 bg-slate-950/80 p-12 rounded-[3rem] border border-white/10 backdrop-blur-md shadow-2xl">
              <div className="flex flex-col items-center">
                <h1 className="text-5xl md:text-7xl font-black tracking-tighter text-blue-500 italic drop-shadow-[0_0_15px_rgba(59,130,246,0.5)]">
                  GORBOY II
                </h1>
                <p className="text-slate-400 font-mono tracking-widest text-sm mt-2">RETRO_FUTURES_LAB // V3.0</p>
              </div>

              <Button
                className="h-16 px-16 text-xl rounded-full bg-blue-600 hover:bg-blue-500 text-white shadow-[0_0_30px_rgba(37,99,235,0.4)] animate-pulse"
              ...
              <div className="text-[10px] text-slate-500 font-mono uppercase tracking-widest">
                Verify Neural Link Before Launch
              </div>
            </div>
          </MotionDiv>
        )}
      ...
      {/* GAME HUD - Only visible when PLAYING (or if we want HUD in background of home, but user flow suggests clean Home) */}
      {/* We wrap the existing header and sidebar in a conditional or just let them fade in */}

      {
        ...
        `}>
                ...
                <Card className="p-4 bg-slate-950/80 border-slate-800">
                  <h2 className="text-[9px] font-black text-slate-500 uppercase tracking-widest mb-3">Ship Configuration</h2>
                  <div className="space-y-2">
                    <div className="flex justify-between items-center text-[10px] text-slate-400 font-mono">
                      <span>MODEL_SCALE</span>
                      <span className="text-slate-200">{playerScale.toFixed(1)}x</span>
                    </div>
                    <input
                      type="range"
                      min="0.5"
              ...
              <main className="flex-1 flex flex-col min-h-0 relative">
                <div ref={containerRef} className="relative flex-1 bg-transparent rounded-[1.5rem] md:rounded-[2.5rem] border border-slate-800/50 overflow-hidden shadow-2xl">
              ...
              <div className="flex flex-col gap-3">
                {MOCK_CHARACTERS.map(c => (
                  <button
                    ...
                    className={`group p-4 rounded-2xl border text-left transition-all duration-300 ${selectedCharacterId === c.id
